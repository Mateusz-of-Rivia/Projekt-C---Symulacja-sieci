...
template<class Node>
void Factory::remove_receiver(NodeCollection<Node>& collection, ElementID_t id) {
...
void Factory::remove_ramp(ElementID_t id) {
    ramps_list.remove_by_id(id);
}

void Factory::remove_worker(ElementID_t id) {
...
void Factory::remove_storehouse(ElementID_t id) {
    remove_receiver(workers_list, id);
